...
const CollateralPreview = ({ settings }: CollateralPreviewProps) => {
  ...
  const getTextStyles = () => {
    return {
      ...
      backgroundColor: settings.textStyle.highlight ? settings.textStyle.highlightColor : 'transparent',
  ...
  // Create an outer container with a background image
  const backgroundContainerStyle: React.CSSProperties = {};
  
  // Add background image if it exists (applied only to the background)
  if (settings.backgroundImageUrl) {
    ...
    backgroundContainerStyle.borderRadius = '20px';
    backgroundContainerStyle.zIndex = 0;
  }

  // These styles override the layout styles to position exactly as required
  const fixedPositionStyles = {
    tableNumber: {
      position: 'absolute',
      top: '40px',
      left: '40px',
      fontSize: `${settings.fontSize.tableNumber}px`,
      ...getTextStyles()
    } as React.CSSProperties,
    actionText: {
      position: 'absolute',
      top: '120px', // Increased from 100px to 120px to move down
      left: '40px',
      fontSize: `${Math.max(30, settings.fontSize.actionText)}px`,
      ...getTextStyles()
    } as React.CSSProperties,
    qrCode: {
      position: 'absolute',
      top: '200px', // Increased from 180px to 200px to move down
      left: '40px',
    } as React.CSSProperties
  };
  ...
  return (
    <div 
      className="rounded-3xl overflow-hidden shadow-xl w-full max-w-md mx-auto relative"
    ...
    >
      ...
      <div className="relative aspect-[3/4] p-4" style={{ fontFamily: settings.fontFamily, position: 'relative', zIndex: 1 }}>
        {settings.logoUrl && (
          <img 
        ...
        {/* Table Number */}
        <div style={fixedPositionStyles.tableNumber}>
          {settings.tableNumber}
        </div>
        
        {/* Action Text */}
        <div style={fixedPositionStyles.actionText}>
          {settings.actionText}
        </div>
        ...
        {settings.venueName && (
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 text-center" 
               style={{ fontSize: `${settings.fontSize.venueName}px`, ...getTextStyles() }}>
            {settings.venueName}
          </div>
        )}
      </div>
